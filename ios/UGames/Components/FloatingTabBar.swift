...
struct FloatingTabBar: View {
    ...
    var body: some View {
            ...
            ForEach(tabs) { tab in
                ...
                VStack(spacing: UGSpace.xs) {
                    Image(systemName: tab.systemIcon)
                        ...
                        .foregroundColor(active ? UGColor.Accent.primary : UGColor.Text.muted)
                    Text(tab.label)
                        .font(UGFont.caption)
                        .foregroundColor(active ? UGColor.Accent.primary : UGColor.Text.muted)
                }
        ...
        .background(UGColor.Surface.subtle.opacity(0.55))
        .clipShape(RoundedRectangle(cornerRadius: UGRadius.xxl))
        .overlay(RoundedRectangle(cornerRadius: UGRadius.xxl).stroke(UGColor.Border.divider))
